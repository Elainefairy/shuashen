...
void init_camera(struct camera *cam)
{
	...
	CLEAR (*cropcap);
        ...
        CLEAR (*fmt);
	/* djc
	if(xioctl(cam->fd, VIDIOC_G_FMT, fmt) == -1) {
		fprintf(stderr, "VIDIO_G_FMT  --  could not get fmt info, exiting....\n");
		if(EINVAL==errno) printf("EINVAL\n");
		if(EBUSY==errno) printf("EBUSY\n");
		exit(0);
	}
	cam->width  = fmt->fmt.win.w.width;
	cam->height = fmt->fmt.win.w.height;
	*/
...
printf("width=%d height=%d\n",cam->width,cam->height);
	//fmt->fmt.win.w.width     = cam->width;
	//fmt->fmt.win.w.height    = cam->height;
        if (-1 == xioctl (cam->fd, VIDIOC_S_FMT, fmt))
                errno_exit ("VIDIOC_S_FMT");
printf("sys: width=%d height=%d\n",fmt->fmt.pix.width,fmt->fmt.pix.height);
...
printf("format: V4L2_PIX_FMT_MJPEG\n");
break;
case V4L2_PIX_FMT_YUYV:
printf("format: V4L2_PIX_FMT_YUYV\n");
break;
default:
printf("format number 0x%X.\n",fmt->fmt.pix.pixelformat);
break;
}
...
{
	if(ioctl(cam->fd, VIDIOC_G_FMT, &cam->video_win) == -1) {
...
{
	if (ioctl(cam->fd, VIDIOC_S_FMT, &cam->video_win) == -1) {
